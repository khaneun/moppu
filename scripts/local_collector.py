...
def _schtasks(args: list[str]) -> tuple[bool, str]:
    """schtasks.exe 실행 — PowerShell cmdlet 호환성 문제 없음."""
    import subprocess
...
def register_task_scheduler(run_time: str = "00:05") -> bool:
    ...
    ok, out = _schtasks([
        "/Create",
        "/TN", "MoppuLocalCollector",
        "/TR", tr,
        "/SC", "DAILY",
        "/ST", run_time,
        "/RL", "HIGHEST",   # 높은 권한으로 실행
        "/F",               # 기존 작업 덮어쓰기
    ])
    if ok:
        log.info(f"✓ Task Scheduler 등록 완료: MoppuLocalCollector (매일 {run_time})")
    else:
        log.warning(f"Task Scheduler 등록 실패: {out[:200]}")
    return ok
...
def register_watch_startup() -> bool:
    ...
    python_exe  = sys.executable
    script_path = str(Path(__file__).resolve())
    tr          = f'"{python_exe}" "{script_path}" --watch'

    ok, out = _schtasks([
        "/Create",
        "/TN", "MoppuLocalCollectorWatch",
        "/TR", tr,
        "/SC", "ONLOGON",
        "/RL", "HIGHEST",
        "/F",
    ...
    if ok:
        log.info("✓ 감시 모드 등록 완료: MoppuLocalCollectorWatch (로그인 시 자동 시작)")
    else:
        log.warning(f"감시 모드 등록 실패: {out[:200]}")
    return ok
