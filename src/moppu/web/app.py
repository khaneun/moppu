...
# 요약 재처리 작업 상태: video_id → {"status": "processing"|"done"|"failed",
#                                      "started_at": iso, "finished_at": iso|None,
#                                      "error": str|None}
# daily_summary 파일은 같은 KST 날짜의 영상끼리 같은 파일을 공유하므로
# 동시 실행 시 race condition 발생 → 직렬 lock 으로 1건씩 처리.
_retry_summary_jobs: dict[str, dict[str, Any]] = {}
_retry_summary_run_lock = threading.Lock()      # 실제 처리는 1건씩
_retry_summary_state_lock = threading.Lock()    # _retry_summary_jobs 보호
...
def _retry_summary_set_state(video_id: str, **fields: Any) -> dict[str, Any]:
    with _retry_summary_state_lock:
        cur = _retry_summary_jobs.get(video_id, {})
        cur.update(fields)
        _retry_summary_jobs[video_id] = cur
        return dict(cur)
...
def retry_summary_only(video_id: str):
    ...
    여러 건 동시에 요청해도 daily_summary 파일 race condition 을 피하기 위해
    백그라운드에서 1건씩 직렬 처리한다. 진행 상태는 GET 으로 조회 가능.
    """
    assert _rt is not None
    with _rt.session_factory() as s:
        v = s.query(Video).filter_by(video_id=video_id).one_or_none()
    ...
    # 이미 큐에 있거나 처리 중이면 중복 등록 방지
    with _retry_summary_state_lock:
        cur = _retry_summary_jobs.get(video_id)
        if cur and cur.get("status") in {"queued", "processing"}:
            return {"ok": True, "status": cur["status"], "message": f"{video_id} 이미 진행 중"}
        _retry_summary_jobs[video_id] = {
            "status": "queued",
            "started_at": datetime.now(KST).isoformat(),
            "finished_at": None,
            "error": None,
    ...
    def _do() -> None:
        # lock 으로 직렬화 — 같은 날짜의 다른 영상과 daily_summary 충돌 방지
        with _retry_summary_run_lock:
            _retry_summary_set_state(video_id, status="processing")
            from moppu.agent.daily_summary import generate_and_save
            from moppu.agent.persona import update_with_new
            try:
    ...
    return {"ok": True, "status": "queued", "message": f"{video_id} 요약·페르소나 재처리 큐 등록"}


@app.get("/api/pipeline/retry-summary/status")
def retry_summary_status(ids: str = ""):
    """요약 재처리 작업 상태 조회.

    ``ids`` 는 콤마로 구분된 video_id 목록. 비어 있으면 전체 반환.
    응답: ``{video_id: {status, started_at, finished_at, error}}``
    """
    with _retry_summary_state_lock:
        if not ids:
            return {vid: dict(s) for vid, s in _retry_summary_jobs.items()}
        wanted = [v.strip() for v in ids.split(",") if v.strip()]
        return {vid: dict(_retry_summary_jobs[vid]) for vid in wanted if vid in _retry_summary_jobs}
