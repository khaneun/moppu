...
def test_stale_token_500_triggers_refresh_and_retry(tmp_path):
    """무효 토큰으로 500 → 토큰 재발급 후 1회 재시도해 성공해야 한다."""
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if request.url.path.endswith("/inquire-balance"):
            ...
            # 첫 토큰(tok1)으로 오는 요청은 무효 → 500.
            if "tok1" in auth:
                return httpx.Response(500, text="EGW00123 만료된 token")
    ...
    assert state["token_seq"] == 2, "토큰이 재발급되어야 한다"
    assert state["balance_calls"] == 2, "잔고조회는 정확히 1회 재시도되어야 한다"
